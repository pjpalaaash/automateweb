...
export default function Lighting() { const [mood, setMood] = useState('Dinner'), [hue, setHue] = useState(32), [bright, setBright] = useState(72); function select(name: string) { const m = moods.find(x => x.name === name); if (m) {
    setMood(m.name);
    setHue(m.h);
    setBright(m.b);
